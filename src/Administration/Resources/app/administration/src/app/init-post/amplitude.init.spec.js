...
jest.mock('@amplitude/analytics-browser', () => amplitudeMock);

describe('src/app/post-init/amplitude.init.ts', () => {
    beforeEach(() => {
        Object.keys(amplitudeMock).forEach((key) => {
            if (typeof amplitudeMock[key]?.mockClear === 'function') {
                amplitudeMock[key].mockClear();
            }
        });
        global.Shopware = {
            ...global.Shopware,
    ...
    describe('initialization', () => {
        it('add enrichment plugin and calls initialization routine', async () => {
            ...
            expect(amplitudeMock.add).toHaveBeenCalled();
            expect(amplitudeMock.add).toHaveBeenCalledWith(
            ...
            expect(amplitudeMock.init).toHaveBeenCalled();
        ...
        it('should execute enrichment plugin with route properties when router is available', async () => {
            ...
            const enrichmentPlugin = amplitudeMock.add.mock.calls[0][0];
    ...
    describe('event handling', () => {
        it.each([
            ...
            expect(amplitudeMock.track).toHaveBeenCalledWith(trackedData.eventName, trackedData.properties);
        });
    });
    ...
    describe('user identification', () => {
        ...
        it('should set user ID in format "shopId:userId"', async () => {
            ...
            expect(amplitudeMock.setUserId).toHaveBeenCalledWith(`${testShopId}:${testUserId}`);
        });
        ...
        it('should update user ID when a different user identifies', async () => {
            ...
            amplitudeMock.setUserId.mockClear();
            ...
            expect(amplitudeMock.setUserId).toHaveBeenCalledWith(`${testShopId}:${anotherUserId}`);
        });
    });
    ...
    describe('login and logout tracking', () => {
        ...
        it('should track Login event when a identify telemetry event with a different userId arrives', async () => {
            ...
            amplitudeMock.setUserId.mockImplementation((userId) => {
                amplitudeUserId = userId;
            });
            amplitudeMock.getUserId.mockImplementation(() => amplitudeUserId);
            ...
            expect(amplitudeMock.track).toHaveBeenCalledWith('Login');
        ...
        it('should track Logout event when a reset telemetry event arrives', async () => {
            ...
            expect(amplitudeMock.track).toHaveBeenCalledWith('Logout');
        });
        ...
        it('should call flush and reset after Logout event', async () => {
            ...
            expect(amplitudeMock.flush).toHaveBeenCalled();
