...
class Migration1648709176CartCompression extends MigrationStep
{
    ...
    public function updateDestructive(Connection $connection): void
    {
        ...
        if (!DbTableHelper::columnExists($schemaManager, 'cart', 'compressed')) {
        ...
        if (!DbTableHelper::columnExists($schemaManager, 'cart', 'payload')) {
            $connection->executeStatement('ALTER TABLE `cart` ADD `payload` LONGBLOB NULL;');
        }

        if (!DbTableHelper::columnExists($schemaManager, 'cart', 'cart')) {
            return;
        }
