...
class Migration1713345551AddAppManagedColumnTest extends TestCase
{
    ...
    {
        static::assertFalse(DbTableHelper::columnExists($this->connection->createSchemaManager(), 'app', 'self_managed'));

        $migration = new Migration1713345551AddAppManagedColumn();
        $migration->update($this->connection);
        $migration->update($this->connection);

        static::assertTrue(DbTableHelper::columnExists($this->connection->createSchemaManager(), 'app', 'self_managed'));
    }
}
