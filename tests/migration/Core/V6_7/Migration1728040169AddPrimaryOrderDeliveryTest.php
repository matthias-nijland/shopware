...
class Migration1728040169AddPrimaryOrderDeliveryTest extends TestCase
{
    ...
    public function testMigration(): void
    {
        ...
        static::assertTrue(DbTableHelper::columnExists($schemaManager, OrderDefinition::ENTITY_NAME, 'primary_order_delivery_id'));
        static::assertTrue(DbTableHelper::columnExists($schemaManager, OrderDefinition::ENTITY_NAME, 'primary_order_delivery_version_id'));
    ...
    private function rollback(MySQLSchemaManager $schemaManager): void
    {
        ...
        if (DbTableHelper::columnExists($schemaManager, 'order', 'primary_order_delivery_id')) {
            $this->connection->executeStatement('ALTER TABLE `order` DROP COLUMN `primary_order_delivery_id`');
        }

        if (DbTableHelper::columnExists($schemaManager, 'order', 'primary_order_delivery_version_id')) {
